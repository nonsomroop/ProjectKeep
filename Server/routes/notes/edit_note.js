...
router.put("/:noteId", (req, res) => {
  ...
  const updateNoteQuery = `UPDATE notes SET title = ?, description = ?, latitude = ?, longitude = ?, created_at = ?, priority = ?, reminder = ?, content = ? WHERE id = ?`;
  const noteValues = [
    title,
    description,
  ...
  connection.query(
    ...
    (err, noteResult) => {
      if (err) {
        ...
        // Delete existing tags and categories associated with the note
        ...
        // Insert new tags into the 'tags' table (ignore if already exists)
        ...
        Promise.all(tagTableCheck)
          .then((results) => {
            ...
            // Insert new categories into the 'categories' table (ignore if already exists)
            ...
            const categoryTableCheck = categories.map((category) => {
              return new Promise((resolve, reject) => {
                connection.query(checkCategories, [category], (err, categoryCheckResult) => {
                  if (err) {
                    ...
                    if (categoryCheckResult.length > 0) {
                      ...
                      connection.query(insertCategoryQuery, [category], (err, categoryResult) => {
                        if (err) {
              ...
              .then((results) => {
                ...
                Promise.all([...addTags, ...addCategories])
                  ...
                  .catch((error) => {
                    ...
                    console.error("Error creating tags or categories:", error);
                    return res.status(500).json({
                      success: false,
                      data: null,
                      error: "An error occurred while creating tags or categories.",
                    });
                  });
              })
              .catch((err) => {
